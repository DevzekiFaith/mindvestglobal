...
`;
  ...
  // Page 2 Stream Content (Frameworks + Contact Box, NO Section 3)
  const p2Stream = `
q
0.058 0.09 0.164 rg
0 740 612 102 re f
...
`;
  ...
  addObj("<< /Type /Pages /Kids [3 0 R 5 0 R] /Count 2 >>");
  addObj("<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Contents 4 0 R /Resources << /Font << /F1 7 0 R /F2 8 0 R >> >> >>");
  const p1Clean = p1Stream.trim();
  addObj(`<< /Length ${Buffer.byteLength(p1Clean, "latin1")} >>\nstream\n${p1Clean}\nendstream`);
  addObj("<< /Type /Page /Parent 2 0 R /MediaBox [0 0 612 792] /Contents 6 0 R /Resources << /Font << /F1 7 0 R /F2 8 0 R >> >> >>");
  const p2Clean = p2Stream.trim();
  addObj(`<< /Length ${Buffer.byteLength(p2Clean, "latin1")} >>\nstream\n${p2Clean}\nendstream`);
